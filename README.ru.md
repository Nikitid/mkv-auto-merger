...
необходимости выполняет remux через `mkvmerge` и создаёт структуру
`Season XX`.
...
удалять исходные файлы. Проверяйте пути перед запуском, начинайте с сухого
прогона и держите резервную копию: отменить перемещение или удаление
инструмент не умеет.
...
Окружение, проверки и выпуск: [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md).

## Документация

- [Карта репозитория](docs/MAP.md) - где что лежит
- [Разработка](docs/DEVELOPMENT.md) - окружение, проверки и выпуск

## Поддержка

Вопросы и сообщения об ошибках - в
[Issues](https://github.com/Nikitid/mkv-auto-merger/issues/new/choose), выберите
подходящую форму. Об уязвимости сообщайте приватно через
[security advisory](https://github.com/Nikitid/mkv-auto-merger/security/advisories/new).
